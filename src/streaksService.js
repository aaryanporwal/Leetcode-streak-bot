...
function getLeaderboard() {
  return db
    .prepare(
      `SELECT user_id, streak, questions_solved FROM users ORDER BY streak DESC`
    )
    .all();
}
...
function getRecentQuestions(userId, limit = 3) {
  return db
    .prepare(
      `SELECT question_name FROM user_questions WHERE user_id = ? ORDER BY timestamp DESC LIMIT ?`
    )
    .all(userId, limit);
}
